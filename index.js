...
// Dev-only public upload (no auth) - enable by setting ALLOW_PUBLIC_UPLOAD=true in .env
if (process.env.ALLOW_PUBLIC_UPLOAD === "true") {
  app.post("/file/upload-public", upload.single("file"), async (req, res) => {
    try {
      const filePath = req.file.path;
      const loader = new PDFLoader(filePath);
      const docs = await loader.load();
      ...
      const chunks = await splitter.splitDocuments(docs);
      console.log("[public-upload] Total chunks:", chunks.length);
      await embedAndStore(chunks, req.file.originalname);
      await deleteLocalFile(filePath);
      res.status(200).json({ message: "File uploaded successfully (public)" });
    } catch (err) {
      console.log(err);
      res.status(500).json({ error: "Failed to upload file" });
    }
